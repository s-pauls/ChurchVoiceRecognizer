...
class SettingsManager:
    ...
    def get_audio_device(self) -> Optional[int]:
        """Возвращает сохраненный индекс аудио устройства"""
        settings = self.load_settings()
        return settings.get("audio_device_index")
        
    def get_service_type(self) -> str:
        """Возвращает сохраненный тип службы"""
        settings = self.load_settings()
        return settings.get("service_type", "литургия")
        
    def save_audio_device(self, device_index: int, device_name: str = "") -> bool:
        """Сохраняет выбранное аудио устройство"""
        settings = self.load_settings()
        settings["audio_device_index"] = device_index
        settings["audio_device_name"] = device_name
        return self.save_settings(settings)
        
    def save_service_type(self, service_type: str) -> bool:
        """Сохраняет тип службы"""
        settings = self.load_settings()
        settings["service_type"] = service_type
        return self.save_settings(settings)
        
    def save_complete_settings(self, device_index: int, device_name: str, service_type: str) -> bool:
...
def save_settings(device_index: int, device_name: str, service_type: str) -> bool:
    """Удобная функция для сохранения настроек"""
    return settings_manager.save_complete_settings(device_index, device_name, service_type)
...
if __name__ == "__main__":
    ...
    if save_settings(0, "Test Device", "всенощная"):
